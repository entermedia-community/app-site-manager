...
public class StripePaymentProcessor 
{
	...
	protected void process(MediaArchive inStore, User inUser, Data inOrder,  String inToken) 
	{
		log.info("processing order with Stripe");
		
		Map<String, Object> chargeParams = new HashMap<String, Object>();
		Money totalprice = new Money(inOrder.get("totalprice"));
		...
		boolean forcetestmode = Boolean.parseBoolean(inOrder.get("forcetestmode"));
		boolean productionmode = inStore.isCatalogSettingTrue("productionmode");
		if(productionmode && !forcetestmode){
			Stripe.apiKey  = inStore.getCatalogSettingValue("stripe_access_token");
		} else{
			Stripe.apiKey  = inStore.getCatalogSettingValue("stripe_test_access_token");
		}
		String amountstring = totalprice.toShortString().replace(".", "").replace("$", "").replace(",", "");
		chargeParams.put("amount", amountstring);
		String currency = inStore.getCatalogSettingValue("currency");
		if(currency == null){
			currency = "usd";
		}
		chargeParams.put("currency", currency);
		String descriptor = inStore.getCatalogSettingValue("statement_descriptor");
		if(descriptor != null){
			chargeParams.put("statement_descriptor", descriptor);
		}	
		Map<String,String> initialMetadata = new HashMap<String,String>();
		populateMetadata(inOrder,inUser,initialMetadata);
		chargeParams.put("description",inOrder.getId());
		...
		try
		{
			...
			if(customerid == null){
				...
				inStore.getUserManager().saveUser(inUser);
			}				
